...
OwnID integrates with Salesforce Commerce Cloud (SFCC) through their built-in API client.
## Prerequisites
...
Saleforce displays your `Client Id` and `Secret`. Save these values for later.
...
<Note> Change `client_id` value in the JSON to match the `Client Id` created in the previous step. </Note>
...
<Info>**Note**: If the OCAPI configuration is already in place, simply include the following resource into the resources section. </Info>
