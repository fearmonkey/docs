...
First, decide on a subdomain. If your domain is `example.com`, we recommend you use `passwordless.example.com` as your custom subdomain, but you can replace `passwordless` with whatever you like as long as it's a valid subdomain.
...
TIP You can use a [DNS Lookup](https://mxtoolbox.com/DnsLookup.aspx) tool and enter your domain. It might tell you who your DNS provider is (but not always).

### Create required DNS records

These are general instructions for creating your DNS records. If you have issues with any of the steps, contact your DNS provider for more assistance.
