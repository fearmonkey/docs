...
Below is an outline of the three major steps required to implement login workflow end-to-end. 

<Steps>
  <Step title="Preparation">
  _[Set up your account](https://console.ownid.com/login), get your application id, and add a field to your user table to store OwnID data._ [Preparation detail>>](#preparation)
  </Step>
  <Step title="Add request handlers to web server">
  _These three event handlers manage requests to each of three end-points made by the OwnID server when it receives login events from your frontend._ [Request handler detail>>](#add-web-server-endpoints)   
  </Step>
  <Step title="Create frontend workflow">
  _Drop in our HTML and JavaScript templates to build the login workflow and install the widget._ [Request handler detail>>](#create-frontend-workflow) 
    
  </Step>
  
</Steps>

The interprocess communication after integration looks like this:
<img
...
<Note>Skip to [Step 2](#add-web-server-endpoints) if you already have the account related information listed below. </Note>
...
2. As part of the account sign up process, you'll establish the parameters needed for integration:
<Info>
...
- Your **base URL** - _You'll establish this as part of creating your account in the console. The OwnID platform uses this to call endpoints on your backend web server._
</Info>
