...
Whether you use your existing _registration_ form or build a new one, the forms in the snippets below are only shown to emphasize the following guidelines. 

#### Registration Form Guidelines

1. Your registration form must contain the fields you want to designate as the _user id_ and _password_. The snippet below uses 'Email' and 'Password' only as examples.
...
<Note>
The following code snippets arbitrarily use `password` and `email` as password and user id. You can use any fields you want in those roles by assigning them as noted above.
</Note>

3. In the 'register' method, the `passwordFieldBinding` parameter determines whether the OwnID platfrom creates a password automatically. The values is `false` by default. If you set it to `true`, OwnID creates a random strong password.
