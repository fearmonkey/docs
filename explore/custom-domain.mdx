...
You can use your own domain name in OwnID flows to maintain seamless branding (for example when users scan a QR Code to sign-in). By configuring a custom domain,  the login experience is unified with your own brand, so your users see a brand-appropriate URL such as `passwordless.your_domain.com` instead of `passwordless.ownid.com`.
