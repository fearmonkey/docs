...
Here, your existing web server hosts the endpoints. We provide code templates for each related event handler.

When a user logs in, your frontend generates a login event and the OwnID server makes the appropriate HTTP requests to _your_ web server, invoking request handlers provided by OwnID. 
