...
This endpoint handles a request to save an `ownIdData` record for a given login id when the user enrolls a new device. This is the VARCHAR field you created above. It holds the public key of the device used for biometric authentication.
...
Note that `token` is a value that you generate (usually a jwt). Later, you will learn how the OwnID login event returns this value in the frontend to establish a client session.
<CodeGroup>
...
<Card title="Build Frontend Flows" href="/building-blocks/build-frontend-integration">
