...
<Steps>
  ...
  _[Set up your account](https://console.ownid.com/login), get your application id, and add a field to your user table to store OwnID data._ [Preparation details>>](#preparation)
  </Step>
  <Step title="Add request handlers to web server">
  _These three event handlers manage requests to each of three end-points made by the OwnID server when it receives login events from your frontend._ [Request handler details>>](#add-web-server-endpoints)   
  </Step>
...
#### Endpoint 1 - Saving a user to an OwnIdData Record
