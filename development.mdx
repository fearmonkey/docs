...
Placing the widget next to the password field (side-by-side)
...
Auto-registration when the account doesnt exist
As part of `/setOwnIDDataByLoginId` endpoint implementation

Establishing the session on `onLogin`
○Direct:

{ onLogin({ token }) { document.cookie = `sessionId=${token}`; } }

Exchange: [when session is an `http-only` cookie]
{ onLogin({ token }) { fetch(`/auth/exchange?token=${token}`); } }
